resource "azurerm_linux_web_app" "linux_web_app" {
  ...
  dynamic "site_config" {
    ...
    content {
      always_on             = try(site_config.value.linux_webapp_always_on, true)
      ...
      dynamic "application_stack" {
        ...
        content {
          dotnet_version   = try(application_stack.value.dotnet_version, "6.0")
          go_version       = try(application_stack.value.go_version, "1.19")
          java_server      = try(application_stack.value.go_version, "JAVA")
          docker_image     = try(application_stack.value.docker_image, "nginx")
          docker_image_tag = try(application_stack.value.docker_image, "latest")
        }
      }
